...
<body>
  <div style="max-width:580px;margin:0 auto;padding:0 20px 40px;">
    ...
    <!-- Header -->
    <div style="padding:32px 0 24px;display:flex;align-items:center;gap:10px;border-bottom:1px solid #1a1a2e;">
      <div style="width:28px;height:28px;background:#6366f1;border-radius:7px;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:white;">N</div>
      <span style="font-size:14px;font-weight:700;color:#e8e8f5;letter-spacing:-0.3px;">NEXUS</span>
    </div>
    ...
    <!-- Footer -->
    <div style="margin-top:48px;padding-top:24px;border-top:1px solid #1a1a2e;font-size:11px;color:#4a4a6a;line-height:1.6;">
      <p>Nexus AI — Your marketing brain.</p>
      <p style="margin-top:4px;">
        <a href="${APP_URL}/settings" style="color:#4a4a6a;">Manage preferences</a>
        &nbsp;·&nbsp;
        <a href="${APP_URL}/privacy" style="color:#4a4a6a;">Privacy</a>
      </p>
    </div>
...
function btn(text: string, url: string): string {
  return `<a href="${url}" style="display:inline-block;margin-top:24px;padding:12px 24px;background:#6366f1;color:white;font-size:13px;font-weight:700;border-radius:10px;text-decoration:none;">${text}</a>`
}
...
function p(text: string, muted = false): string {
  return `<p style="font-size:14px;color:${muted ? '#6a6a8a' : '#b8b8d8'};line-height:1.7;margin-bottom:12px;">${text}</p>`
}

function card(content: string): string {
  return `<div style="background:#111120;border:1px solid #1e1e38;border-radius:12px;padding:20px 24px;margin:20px 0;">${content}</div>`
}
...
export async function sendWelcomeEmail(to: string, name: string) {
  ...
  const content = `
    ...
    ${card(`
      ...
      <div style="display:flex;flex-direction:column;gap:12px;">
        <div style="display:flex;align-items:flex-start;gap:12px;">
          <div style="width:22px;height:22px;background:#6366f1;border-radius:6px;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:900;color:white;flex-shrink:0;">1</div>
          <div>
        ...
        <div style="display:flex;align-items:flex-start;gap:12px;">
          <div style="width:22px;height:22px;background:#6366f1;border-radius:6px;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:900;color:white;flex-shrink:0;">2</div>
          <div>
        ...
        <div style="display:flex;align-items:flex-start;gap:12px;">
          <div style="width:22px;height:22px;background:#6366f1;border-radius:6px;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:900;color:white;flex-shrink:0;">3</div>
          <div>
    ...
    <div style="background:#0d0d1a;border:1px solid #2a2a4a;border-radius:10px;padding:16px 20px;margin:20px 0;">
      <div style="font-size:12px;color:#6366f1;font-weight:700;margin-bottom:4px;">⚡ You have 30 free AI credits</div>
      ...
      <div style="display:flex;flex-direction:column;gap:8px;">
        ${['200 AI credits every month', 'Unlimited campaigns', 'Weekly strategy brief in your inbox', 'Social publishing to Instagram & Facebook', 'PDF campaign reports'].map(f =>
          `<div style="display:flex;align-items:center;gap:8px;font-size:13px;color:#b8b8d8;">
            <span style="color:#6366f1;font-weight:700;">✓</span> ${f}
          </div>`
        ).join('')}
      </div>
    `)}
...
export async function sendWeeklyBrief(to: string, data: WeeklyBriefData) {
  ...
  const ideasHtml = data.contentIdeas.map((idea, i) => `
    <div style="display:flex;align-items:flex-start;gap:12px;padding:12px 0;border-bottom:1px solid #1a1a2e;">
      <div style="width:20px;height:20px;background:#1e1e38;border:1px solid #2a2a4a;border-radius:5px;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;color:#6366f1;flex-shrink:0;">${i + 1}</div>
      <div style="font-size:13px;color:#b8b8d8;line-height:1.5;">${idea}</div>
    </div>
  `).join('')

  const content = `
    <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#6366f1;margin-bottom:8px;">Weekly Brief · ${weekOf}</div>
    ...
    ${card(`
      <div style="font-size:12px;color:#6366f1;font-weight:700;text-transform:uppercase;letter-spacing:1px;margin-bottom:12px;">🎯 This week's focus</div>
    ...
    <div style="margin-top:24px;">
      ...
      <div style="border:1px solid #1e1e38;border-radius:10px;overflow:hidden;margin-top:12px;">
        ${ideasHtml}
        <div style="padding:12px 0;"></div>
      </div>
    </div>

    <div style="margin-top:20px;background:#0d0d1a;border:1px solid #1e1e38;border-radius:10px;padding:16px 20px;display:flex;justify-content:space-between;align-items:center;">
      <div>
      ...
      <div>
        ...
        <div style="font-size:18px;font-weight:800;color:#6366f1;">Active</div>
      </div>
    </div>
...
export async function sendUpgradeConfirmationEmail(to: string, name: string, plan: string) {
  ...
  const content = `
    ...
    <div style="background:#0d1f0d;border:1px solid #1a3a1a;border-radius:10px;padding:16px 20px;margin:20px 0;">
