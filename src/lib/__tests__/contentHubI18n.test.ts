...
describe('Content Hub i18n dictionary', () => {
  ...
  it('9. per-post action hierarchy uses review-only safety copy and compact action labels', () => {
    ...
    expect(translations.en.contentHub.rewriteCopyShort).toBe('Rewrite — 2 credits')
    expect(translations.en.contentHub.generateImageShort).toBe('Image — 4 credits')
    ...
    expect(translations.ar.contentHub.generateImageShort).toMatch(/4|٤/)
  })
})
