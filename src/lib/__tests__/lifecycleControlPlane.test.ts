...
describe('customer lifecycle control-plane contract', () => {
  ...
  it('makes pre-permission truth visible in the operator UI', () => {
    ...
    expect(lifecyclePage).toContain("ar ? 'التسليم مقفل' : 'Delivery blocked'")
    expect(lifecyclePage).toContain('Approved, not sent')
    expect(lifecyclePage).toContain("DOUBLE_OPT_IN: ['تأكيد اشتراك مزدوج — نص فقط', 'Double opt-in — copy only']")
    expect(lifecyclePage).toContain('localizedLabel(DELIVERY_BLOCKER_LABELS, blocker, ar)')
    expect(lifecyclePage).not.toContain('>DELIVERY BLOCKED<')
    expect(lifecyclePage).not.toContain('DOUBLE_OPT_IN (copy only)')
  })
})
