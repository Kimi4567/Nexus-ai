...
describe('strategy pricing display truth', () => {
  it('derives Billing examples from the server pricing resolver', () => {
    expect(STRATEGY_PRICING_DISPLAY_TRUTH.trialActivation.cost).toBe(8)
    expect(STRATEGY_PRICING_DISPLAY_TRUTH.paidStandard90.cost).toBe(16)
    expect(STRATEGY_PRICING_DISPLAY_TRUTH.fullStandard90.cost).toBe(21)
    expect(STRATEGY_PRICING_DISPLAY_TRUTH.range).toEqual({ minimum: 8, maximum: 34 })
  })
