...
describe('cinematic product-ad preflight', () => {
  ...
  it('uses grounded visible evidence when the provider omits an optional product label', () => {
    ...
    expect(assessCinematicProductAdAssets([close, full])).toMatchObject({
      eligible: true,
      route: 'CINEMATIC_PRODUCT_AD',
      qualifiedAssetIds: ['close', 'full'],
    })
  })
