...
function softenAbsoluteOutcomeClaims(text: string): string {
  return text
    ...
    .replace(/اكتشف\s+كيف\s+يمكن\s+لحملاتنا\s+تحسين\s+استفساراتك\.?/giu, 'راجع كيف تُدار صورك ورسائلك في حملة واحدة.')
    .replace(/تحويل\s+صور\s+العقارات\s+المعتمدة\s+إلى\s+حملات\s+محتوى\s+(?:فعالة|فعّالة)\.?/giu, 'تحويل صور العقارات المعتمدة إلى مسودات حملات محتوى قابلة للمراجعة.')
    .replace(/اجعل\s+حملاتك\s+العقارية\s+أكثر\s+(?:فعالية|فاعلية)\.?/giu, 'اجعل حملاتك العقارية أسهل في المراجعة.')
    .replace(/حملات\s+عقارية\s+موثوقة\s+تعتمد\s+على\s+بياناتك(?:\s+المعتمدة)?\.?/giu, 'مسودات حملات عقارية مبنية على بياناتك المعتمدة وقابلة للمراجعة.')
    .replace(/نحن\s+نوفر\s+حملات\s+موثوقة\s+تعتمد\s+على\s+بياناتك\s+المعتمدة\.?/giu, 'نوفّر مسودات حملات مبنية على بياناتك المعتمدة وقابلة للمراجعة.')
    .replace(/نحن\s+نوفر\s+حلولاً\s+متكاملة\s+لإدارة\s+حملاتك\.?/giu, 'نوفّر مساراً واحداً لتخطيط المحتوى ومراجعته داخل NEXUS.')
