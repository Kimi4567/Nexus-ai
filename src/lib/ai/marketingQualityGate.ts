...
const UNSOURCED_CHANNEL_FACT_RE = /\b(?:high(?:est)?[-\s]?engagement|fastest[-\s]?growing|rapidly growing|popular among|best platform|leading platform|dominant platform|most effective platform|(?:is\s+)?effective\s+for|ideal\s+for|can\s+drive\s+(?:traffic|sales|conversions?)|(?:platform\s+)?users?\s+(?:actively\s+)?(?:seek|search\s+for|prefer))\b|(?:تفاعل\s+مرتفع|الأسرع\s+نمواً|ينمو\s+بسرعة|شائع\s+بين|أفضل\s+منصة|المنصة\s+الرائدة|الأكثر\s+فعالية|فع[ّ]?ال(?:ة)?\s+لـ?|مثالي(?:ة)?\s+لـ?|يمكن(?:ها)?\s+زيادة\s+(?:الزيارات|المبيعات|التحويلات)|مستخدمو?\s+\S+\s+(?:يبحثون|يفضلون))/i
