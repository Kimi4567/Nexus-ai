...
function guardDraftCopyQuality(text: string): string {
  return text
    ...
    .replace(
      /(?:شاهد|راجع|اكتشف)?\s*وتأكد\s+من\s+جودة\s+القهوة(?:\s+التي\s+تصلك)?\.?/gi,
      ' راجع تفاصيل القهوة والاشتراك المتاحة.',
    )
    .replace(
      /تأكد\s+من\s+جودة\s+القهوة(?:\s+التي\s+تصلك)?\.?/gi,
      'راجع تفاصيل القهوة والاشتراك المتاحة.',
    )
    ...
    .replace(
      /اكتشف\s+كيف\s+يمكننا\s+مساعدتك\s+في\s+تنظيم(?:ها|ه|هم)\s+بشكل\s+فعال\.?/giu,
      'راجع كيف تُجمع الأصول ورسائل الحملة في مسار واحد قابل للمراجعة.',
    )
    .replace(
      /تأك[ّ]?د\s+من\s+دقة\s+(?:محتوى\s+)?حملاتك\s+العقارية/giu,
      'راجع دقة مسودات حملاتك العقارية',
    )
    .replace(
      /(?:ابدأ\s+الآن\s+)?لضمان\s+محتوى\s+(?:دقيق(?:\s+و\s*)?موثوق|موثوق)/giu,
      'راجع المحتوى قبل اعتماده',
    )
    .replace(
      /(?:تعر[ّ]?ف\s+على\s+التفاصيل\s+)?حول\s+كيفية\s+ضمان\s+محتوى\s+(?:دقيق(?:\s+و\s*)?موثوق|موثوق)/giu,
      'تعرّف على خطوات مراجعة المحتوى قبل اعتماده',
    )
...
function guardUnverifiedCoffeeProductClaims(
  ...
  if (!hasAffirmedBrandFact(
    ...
    guarded = guarded
      .replace(
        /(?:شاهد|اكتشف|تعرّف|تعرف)\s+(?:على\s+)?عملية\s+(?:ال)?تحميص(?:\s+(?:القهوة|الحبوب))?/gi,
        'راجع تفاصيل القهوة المحمصة المتاحة',
      )
      .replace(
        /عملية\s+(?:ال)?تحميص\s*(?:القهوة|الحبوب)?/gi,
