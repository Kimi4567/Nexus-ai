...
export type EvidencePlatform = 'META' | 'LINKEDIN' | 'TIKTOK' | 'YOUTUBE'
...
export function readPerformanceEvidence(value: unknown): PerformanceEvidence | null {
  ...
  if (
    ...
    || !['META', 'LINKEDIN', 'TIKTOK', 'YOUTUBE'].includes(String(data.platform))
...
/**
 * Compatibility guard for legacy analytics consumers. Zero is a valid measured
 * result, while empty objects and workflow-only metadata are not evidence.
 * New learning flows should prefer readPerformanceEvidence(), which also
 * verifies provenance, platform, sample size, and schema version.
 */
export function hasRealPerformanceAnalytics(value: unknown): boolean {
  if (!isMetricRecord(value)) return false
  return PERFORMANCE_METRIC_KEYS.some(
    (key) => numberFromPerformanceMetric(value[key]) !== null,
  )
}
