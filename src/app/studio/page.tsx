...
import { Wand2, Loader2, Film, Copy, Play } from 'lucide-react'
      ...
      <div className="glass p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <div>
            ...
            <select value={style} onChange={(e) => setStyle(e.target.value)} className="input-nexus">
        ...
        <button onClick={handleGenerate} disabled={loading || !product || !description} className="btn-primary">
        ...
        {script && (
          <div className="mt-4 p-4 rounded-xl bg-white/5">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={() => navigator.clipboard.writeText(script)} className="text-xs text-amber hover:text-amber-dark transition-colors">
          ...
          {jobs.map((job) => (
            <div key={job.id} className="flex items-center gap-4 p-4 rounded-xl bg-white/5">
              ...
              <div className="flex items-center gap-3">
                <span className={`text-xs font-medium ${statusColors[job.status]}`}>{statusLabels[job.status]}</span>
                {job.status === 'generating' && (
                  <div className="w-24 h-1.5 bg-white/10 rounded-full overflow-hidden">
                    <div className="h-full bg-cyan rounded-full transition-all" style={{ width: `${job.progress}%` }} />
                  </div>
                )}
                {job.status === 'ready' && (
