    ...
    <AppShell>
      <main dir={dir} className="nx-os-page min-h-screen bg-[#F6F8FC] px-4 py-5 sm:px-6 lg:px-8">
        ...
        <section className="mb-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-7 text-amber-900">
          <div className="flex items-start gap-3"><ShieldCheck className="mt-1 h-5 w-5 shrink-0" /><div><p className="font-black">{ar ? 'وضع ما قبل التصاريح — الإرسال مقفل' : 'Pre-permission mode — sending locked'}</p><p>{ar ? 'Draft يعني مسودة، وApproved يعني اعتماد النص فقط. لا توجد حالة إرسال أو قائمة تسليم أو معرّف مزود. تأكيد الاشتراك هنا مسودة نص فقط ولا يثبت هوية جهة الاتصال قبل ربط مزود وتحقق موثّق.' : 'Draft means draft, and Approved means copy approval only. There is no sent state, delivery queue, or provider reference. Double opt-in is copy-only here and does not verify contact identity until an audited provider and verification callback are connected.'}</p></div></div>
        </section>
