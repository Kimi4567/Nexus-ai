...
export default function LeadLifecyclePage() {
  ...
  async function createDraft(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setNotice({ tone: 'success', text: ar ? 'تم حفظ Draft فقط. لم يتم إرسال أي رسالة.' : 'Draft saved only. No message was sent.' })
      await load()
    } catch (error) {
  ...
  async function approveCopy(message: LifecycleMessage) {
    ...
    try {
      ...
      setNotice({ tone: 'success', text: ar ? 'تم اعتماد النص فقط. التسليم ما زال BLOCKED ولا يوجد إرسال.' : 'Copy approved only. Delivery remains BLOCKED and nothing was sent.' })
      await load()
    } catch (error) {
  ...
  async function copyPreviewPath() {
    ...
    try {
      ...
      setNotice({ tone: 'success', text: ar ? 'تم نسخ رابط اختبار unsubscribe.' : 'Unsubscribe test link copied.' })
    } catch {
    ...
    <AppShell>
      <main dir={dir} className="nx-os-page min-h-screen bg-[#F6F8FC] px-4 py-5 sm:px-6 lg:px-8">
        <LuxuryWorkspaceHeader
          pageTitle={ar ? 'Lifecycle Email & SMS' : 'Lifecycle Email & SMS'}
        ...
        <section className="mb-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-7 text-amber-900">
          <div className="flex items-start gap-3"><ShieldCheck className="mt-1 h-5 w-5 shrink-0" /><div><p className="font-black">{ar ? 'وضع ما قبل التصاريح — sendsEnabled=false' : 'Pre-permission mode — sendsEnabled=false'}</p><p>{ar ? 'Draft يعني مسودة، وApproved يعني اعتماد النص فقط. لا توجد حالة SENT، ولا Queue، ولا Provider ID. Double opt-in هنا مسودة نص فقط ولا يثبت هوية جهة الاتصال قبل ربط مزود وwebhook موثّق.' : 'Draft means draft, and Approved means copy approval only. There is no SENT state, queue, or provider ID. Double opt-in is copy-only here and does not verify contact identity until an audited provider and webhook are connected.'}</p></div></div>
        </section>
        ...
        {previewPath ? <div className="mb-4 flex flex-col gap-3 rounded-2xl border border-indigo-100 bg-indigo-50 p-4 text-xs text-indigo-900 sm:flex-row sm:items-center sm:justify-between"><p><strong>{ar ? 'رابط اختبار فقط:' : 'Test link only:'}</strong> {ar ? 'يختبر مسار unsubscribe ولا يعني أن الرسالة أُرسلت.' : 'Validates the unsubscribe path; it does not mean a message was sent.'}</p><button type="button" onClick={() => void copyPreviewPath()} className="inline-flex h-9 items-center justify-center gap-2 rounded-xl bg-white px-3 font-black"><Clipboard className="h-3.5 w-3.5" />{ar ? 'نسخ' : 'Copy'}</button></div> : null}

        {!ready ? (
          <section className="nx-os-card mx-auto max-w-3xl p-8 text-center"><MailCheck className="mx-auto h-10 w-10 text-[#5E63FF]" /><h2 className="mt-4 text-xl font-black text-[#0B1028]">{ar ? 'Lifecycle غير مفعّل' : 'Lifecycle controls are not enabled'}</h2><p className="mt-2 text-sm leading-7 text-slate-500">{ar ? 'طبّق migration واختبر Preview، ثم اضبط مفاتيح HMAC وفعّل LIFECYCLE_MESSAGING_ENABLED. هذا لا يفعّل الإرسال.' : 'Apply the migration, verify Preview, configure HMAC keys, then enable LIFECYCLE_MESSAGING_ENABLED. This still does not enable sending.'}</p></section>
        ) : (
          <>
            <div className="grid gap-4 xl:grid-cols-2">
              <form onSubmit={createDraft} className="nx-os-card p-5">
                <div className="flex items-center gap-2"><Plus className="h-5 w-5 text-[#5E63FF]" /><h2 className="text-base font-black text-[#0B1028]">{ar ? 'مسودة Lifecycle' : 'Lifecycle draft'}</h2></div>
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <select aria-label={ar ? 'عميل المسودة' : 'Draft lead'} required value={messageForm.leadId} onChange={event => setMessageForm(current => ({ ...current, leadId: event.target.value }))} className="h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold text-slate-700 sm:col-span-2"><option value="">{ar ? 'اختر العميل' : 'Choose lead'}</option>{leads.map(lead => <option key={lead.id} value={lead.id}>{lead.fullName || lead.email || lead.phone || lead.id} · Consent {lead.consentStatus}</option>)}</select>
                  <select aria-label={ar ? 'قناة المسودة' : 'Draft channel'} value={messageForm.channel} onChange={event => setMessageForm(current => ({ ...current, channel: event.target.value as 'EMAIL' | 'SMS', subject: event.target.value === 'SMS' ? '' : current.subject }))} className="h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold text-slate-700"><option value="EMAIL">EMAIL</option><option value="SMS">SMS</option></select>
                  <select aria-label={ar ? 'غرض المسودة' : 'Draft purpose'} value={messageForm.purpose} onChange={event => setMessageForm(current => ({ ...current, purpose: event.target.value }))} className="h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold text-slate-700"><option value="FOLLOW_UP">FOLLOW_UP</option><option value="NURTURE">NURTURE</option><option value="WIN_BACK">WIN_BACK</option><option value="DOUBLE_OPT_IN">DOUBLE_OPT_IN (copy only)</option></select>
              ...
              <form onSubmit={suppressContact} className="nx-os-card p-5">
                ...
                <p className="mt-2 text-xs leading-6 text-slate-500">{ar ? 'يُخزّن HMAC فقط لجهة الاتصال. المنع يظل قائمًا حتى لو حُذف سجل Lead.' : 'Stores only a keyed HMAC of the destination. Suppression survives lead deletion.'}</p>
                <div className="mt-4 grid gap-3">
                  ...
                  <select aria-label={ar ? 'قناة المنع' : 'Suppression channel'} value={suppressionForm.channel} onChange={event => setSuppressionForm(current => ({ ...current, channel: event.target.value as 'EMAIL' | 'SMS' }))} className="h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold text-slate-700"><option value="EMAIL">EMAIL</option><option value="SMS">SMS</option></select>
              ...
              <div className="mt-4 space-y-3">{messages.length === 0 ? <p className="rounded-xl bg-slate-50 p-6 text-center text-sm font-bold text-slate-500">{ar ? 'لا توجد مسودات بعد.' : 'No lifecycle drafts yet.'}</p> : messages.map(message => (
                <article key={message.id} className="rounded-2xl border border-slate-100 bg-white p-4">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-lg px-2.5 py-1 text-[11px] font-black ${message.status === 'APPROVED' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>{message.status === 'APPROVED' ? 'APPROVED COPY' : message.status}</span><span className="rounded-lg bg-slate-100 px-2.5 py-1 text-[11px] font-black text-slate-600">DELIVERY BLOCKED</span><span className="rounded-lg bg-indigo-50 px-2.5 py-1 text-[11px] font-black text-indigo-700">{message.channel}</span></div><p className="mt-3 text-sm font-black text-[#0B1028]">{message.lead.fullName || message.lead.email || message.lead.phone || (ar ? 'عميل بدون اسم' : 'Unnamed lead')} · {message.purpose}</p>{message.subject ? <p className="mt-2 text-xs font-bold text-slate-600">{message.subject}</p> : null}<p className="mt-2 line-clamp-3 whitespace-pre-wrap text-xs leading-6 text-slate-500">{message.body}</p><div className="mt-3 flex flex-wrap gap-2">{(message.delivery?.blockers || ['PROVIDER_NOT_CONNECTED']).map(blocker => <span key={blocker} className="rounded-md bg-slate-100 px-2 py-1 text-[10px] font-black text-slate-500">{blocker}</span>)}</div></div>
