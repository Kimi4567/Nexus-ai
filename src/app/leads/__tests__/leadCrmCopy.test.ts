...
describe('lead CRM product truth', () => {
  it('exposes CRM navigation while keeping migration and automation boundaries explicit', () => {
    ...
    expect(page).toContain('Lead intake: inactive · automated outreach: off')
    expect(page).not.toContain('LEADS_CRM_ENABLED=false')
    expect(page).not.toContain('outreachAutomation=false')
