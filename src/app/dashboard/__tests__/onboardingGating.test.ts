...
describe('Dashboard onboarding gating', () => {
  ...
  it('keeps the first operating action owner-led, strategy-first, and context-aware', () => {
    expect(SRC).toContain('<OwnerCampaignStarterModal')
    expect(SRC).toContain("fetch('/api/campaigns/prepare'")
    expect(SRC).toContain("Prefer: 'respond-async'")
    expect(SRC).toContain("'Idempotency-Key': operationId")
    expect(SRC).toMatch(/Let NEXUS prepare your first campaign/)
    expect(SRC).toMatch(/NEXUS prepares the work; you only review decisions that need you/)
    ...
    expect(SRC).toMatch(/Turn the approved strategy into content/)
    expect(SRC).toMatch(/content preparation starts automatically/)
    expect(SRC).toMatch(/Continue content preparation/)
    expect(SRC).toMatch(/Review copy and media, then record the schedule/)
    expect(SRC).toMatch(/Save copy approval first, then media approval, then internal scheduling as a separate decision/)
    expect(SRC).toContain("href: `/campaigns/${topCampaign.id}/content-hub`")
  ...
  it('keeps Arabic dashboard quantities grammatical and the shell localized', () => {
    ...
    expect(SRC).not.toContain('${campaignCount} campaigns')
    expect(SRC).not.toContain('publish-capable')
    expect(SRC).toContain("campaignCount === 1 ? 'campaign' : 'campaigns'")
    expect(SRC).toContain('derivePlatformReadiness(data.accounts)')
    expect(SRC).toContain('identities saved')
    expect(SRC).toContain('publish-ready')
