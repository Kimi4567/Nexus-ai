    ...
    for (const weekItem of weeklyPlan) {
      ...
      for (const platformStr of platformsToSchedule) {
        ...
        if (userMediaItems.length > 0) {
          const postIndex: number = createdPosts.length
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const post: any = await (prisma.socialPost as any).create({
          data: {
            ...
            pageName,
            caption,
            imagePrompt,          // always stored for reference / AI fallback
            imageUrl: resolvedImageUrl ?? null,   // set if user asset chosen
