...
function buildCampaignHTML(campaign: any): string {
  ...
  const hooksHTML = hooks.length
    ? `<ul>${hooks.map((h: string) => `<li>${escapeHtml(h)}</li>`).join('')}</ul>`
    : '<p class="empty">No hooks generated yet.</p>'

  const captionsHTML = captions.length
    ? `<ul>${captions.map((c: string) => `<li>${escapeHtml(c)}</li>`).join('')}</ul>`
  ...
  const calendarHTML = contentCalendar.length
    ? `<table><thead><tr><th>Day</th><th>Platform</th><th>Content</th></tr></thead><tbody>
        ${contentCalendar.map((item: any) =>
          `<tr><td>${escapeHtml(item.day ?? item.date ?? '—')}</td><td>${escapeHtml(item.platform ?? '—')}</td><td>${escapeHtml(item.content ?? item.caption ?? '—')}</td></tr>`
        ).join('')}
       </tbody></table>`
  ...
  const conceptsHTML = concepts.length
    ? concepts.map((c: any) => `
        <div class="concept-card">
          <h4>${escapeHtml(c.name ?? 'Concept')}</h4>
          ${c.description ? `<p>${escapeHtml(c.description)}</p>` : ''}
          ${c.headline    ? `<p><strong>Headline:</strong> ${escapeHtml(c.headline)}</p>` : ''}
          ${c.body        ? `<p><strong>Body:</strong> ${escapeHtml(c.body)}</p>` : ''}
          ${c.cta         ? `<p><strong>CTA:</strong> ${escapeHtml(c.cta)}</p>` : ''}
        </div>`).join('')
...
<head>
  ...
  <title>${campaignName} — Nexus AI Campaign Report</title>
  <style>
...
<div class="section">
  ...
  <p>${escapeHtml(typeof cta === 'string' ? cta : JSON.stringify(cta))}</p>
</div>` : ''}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const format = typeof body.format === 'string' ? body.format.toUpperCase() : 'HTML'
    const type = typeof body.type === 'string' ? body.type.trim().slice(0, 60) : 'full_campaign'
    if (!campaignId) return NextResponse.json({ error: 'Campaign ID is required' }, { status: 400 })
    if (format !== 'HTML' && format !== 'JSON') {
      return NextResponse.json({ error: 'Supported export formats are HTML and JSON' }, { status: 400 })
    }
