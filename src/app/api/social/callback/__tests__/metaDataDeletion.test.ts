 ...
 *   - GET with hub.challenge echoes the challenge (webhook verification)
 *
...
const { mockFindFirst, mockCreate, mockAdFindFirst } = vi.hoisted(() => ({
  mockFindFirst: vi.fn(),
  mockCreate: vi.fn(),
  mockAdFindFirst: vi.fn(),
}))

vi.mock('@/lib/prisma', () => ({
  prisma: {
    dataDeletionRequest: { findFirst: mockFindFirst, create: mockCreate },
    adAccount: { findFirst: mockAdFindFirst },
  },
}))
...
describe('Meta data-deletion callback', () => {
  ...
  it('GET echoes hub.challenge for webhook verification', async () => {
    const res = await GET({ url: 'https://nexus-grow.com/api/social/callback/meta-data-deletion?hub.challenge=abc123' } as any)
    expect(res.status).toBe(200)
