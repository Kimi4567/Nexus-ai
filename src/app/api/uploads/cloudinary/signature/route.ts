...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!sessionToken) {
      return NextResponse.json(createUploadError(400, 'Upload session required', 'SESSION_REQUIRED'), { status: 400 })
    }
    const session = await prisma.uploadSession.findUnique({ where: { token: sessionToken } })
    if (!session || session.userId !== userId) {
      await logUploadEvent({ userId, sessionId: session?.id, eventType: 'INVALID_UPLOAD_SESSION', severity: 'WARN' })
