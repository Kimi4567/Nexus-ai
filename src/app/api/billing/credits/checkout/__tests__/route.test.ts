...
vi.mock('@/lib/stripe', () => ({
  ...
  getCreditWalletLineItems: vi.fn((credits: number) => credits === 300
    ? [{ price: 'price_tier_1', quantity: 100 }, { price: 'price_tier_2', quantity: 200 }]
...
describe('POST /api/billing/credits/checkout', () => {
  it('uses only the server quote and binds it to signed Stripe metadata', async () => {
    ...
    expect(body.quote).toMatchObject({ credits: 300, amountCents: 6_900 })
    expect(mocks.createSession).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        mode: 'payment',
        line_items: [
          { price: 'price_tier_1', quantity: 100 },
          { price: 'price_tier_2', quantity: 200 },
        ],
        metadata: {
  ...
  it.each([49, 51, 5_010, 100.5])('rejects unsafe quantity %s before Stripe', async (credits) => {
    const response = await POST(request({
      credits,
      requestId: '73d6b695-3bd1-46ec-bcfe-44ccfe8a71a1',
    }))
