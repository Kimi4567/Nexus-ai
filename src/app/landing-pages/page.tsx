...
export default function LandingPagesWorkspace() {
  ...
  const matchingForms = useMemo(() => captureForms.filter(form => (
    Boolean(editor.campaignId)
    && form.status === 'ACTIVE'
    && (form.campaignId === editor.campaignId || form.campaignId === null)
  )), [captureForms, editor.campaignId])
            ...
            {editorVisible ? (
              <section className="nx-os-card mt-4 overflow-hidden">
                ...
                <form onSubmit={showCreate ? createPage : savePage} className="grid gap-4 p-5 lg:grid-cols-2">
                  ...
                  <label className="text-xs font-black text-slate-700">{copy('الحملة *', 'Campaign *')}<select required aria-describedby="landing-campaign-helper" disabled={Boolean(selected?.publishedVersion)} value={editor.campaignId} onChange={event => setEditor(current => ({ ...current, campaignId: event.target.value, captureFormId: '' }))} className="mt-2 h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm disabled:cursor-not-allowed disabled:bg-slate-100"><option value="">{copy('اختر حملة — مطلوب', 'Select campaign — required')}</option>{campaigns.map(campaign => <option key={campaign.id} value={campaign.id}>{campaign.name}</option>)}</select><span id="landing-campaign-helper" className="mt-1.5 block text-[10px] font-bold text-slate-400">{selected?.publishedVersion ? copy('هوية الحملة تثبت بعد أول نشر.', 'Campaign identity is fixed after first publication.') : copy('تحدد الحملة إسناد النموذج والقياس؛ لا يمكن إنشاء المسودة بدونها.', 'The campaign controls form and measurement attribution; a draft cannot be created without it.')}</span></label>
                  ...
                  <label className="text-xs font-black text-slate-700">{copy('نموذج استقبال مرتبط', 'Linked capture form')}<select disabled={!editor.campaignId} value={editor.captureFormId} onChange={event => setEditor(current => ({ ...current, captureFormId: event.target.value }))} className="mt-2 h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm disabled:cursor-not-allowed disabled:bg-slate-100"><option value="">{editor.campaignId ? copy('بدون نموذج', 'No form') : copy('اختر الحملة أولاً', 'Select a campaign first')}</option>{matchingForms.map(form => <option key={form.id} value={form.id}>{form.name}{form.campaignId === null ? copy(' — غير مرتبط؛ سيُربط بهذه الحملة', ' — unassigned; will link to this campaign') : ''}</option>)}</select><span className="mt-1.5 block text-[10px] font-bold text-slate-400">{copy('يمكن اختيار نموذج نشط لنفس الحملة أو نموذج غير مرتبط؛ اختيار النموذج غير المرتبط يربطه بالحملة صراحة عند الحفظ.', 'Choose an active form from this campaign or an unassigned form; selecting an unassigned form explicitly links it when saved.')}</span></label>
