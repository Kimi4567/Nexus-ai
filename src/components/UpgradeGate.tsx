...
export default function UpgradeGate({ feature, description, onClose, inline = false }: UpgradeGateProps) {
  if (inline) {
    return (
      <div className="rounded-2xl border border-[#FF9500]/20 bg-[#FF9500]/5 p-6 text-center"
        style={{ boxShadow: '0 0 32px rgba(255,149,0,0.08)' }}>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4"
      ...
      <div className="w-full max-w-sm rounded-2xl border border-[#FF9500]/20 bg-dark p-8 text-center"
        style={{ boxShadow: '0 0 80px rgba(255,149,0,0.12)' }}>
        {onClose && (
        ...
        <div className="w-14 h-14 rounded-2xl bg-[#FF9500]/10 border border-[#FF9500]/20 flex items-center justify-center mx-auto mb-4 text-2xl">
          ⚡
        </div>
        ...
        <div className="space-y-2">
          ...
          {onClose && (
            <button onClick={onClose}
              className="block w-full py-2.5 border border-dark-tertiary hover:border-s4 text-gray-500 hover:text-white rounded-xl text-sm transition-all">
              Maybe later
            </button>
          )}
        </div>
