...
export default function StrategyDecisionDesk({
  ...
  const handoffLinks = [
    ...
    { key: 'paid', label: text('التنفيذ المدفوع', 'Paid execution'), href: snapshot.scope === 'paid' || snapshot.scope === 'full' ? `/campaigns/${campaign.id}/execution` : snapshot.executionLinks.paid, status: snapshot.scope === 'paid' || snapshot.scope === 'full' ? text('مسار مستقل بموافقات', 'Separate approval-gated lane') : text('خارج النطاق', 'Out of scope'), tone: snapshot.scope === 'paid' || snapshot.scope === 'full' ? 'checking' as Tone : 'muted' as Tone, helper: text('تحويل الحزمة المعتمدة إلى حملة وإعلانات وإبداع وميزانية قابلة للمراجعة.', 'Turns the approved package into reviewable campaigns, ads, creative, and budget.') },
  ...
  const truthBarItems = [
    ...
    { key: 'strategy', label: text('اعتماد الاستراتيجية', 'Strategy approval'), value: strategyStatusLabel, helper: snapshot.approvalState === 'approved' ? text('اعتماد الاستراتيجية مسجل؛ النص والوسائط لهما بوابات مراجعة منفصلة.', 'Strategy approval is recorded; copy and media keep separate review gates.') : qualityState === 'passed' ? text('فحص الجودة ناجح؛ ما زال قرار اعتماد الاستراتيجية مطلوبًا.', 'Quality review passed; the strategy approval decision is still required.') : text('جودة الاستراتيجية أو اعتمادها ما زالا مطلوبين؛ النص والوسائط لهما بوابات منفصلة.', 'Strategy quality or approval is still required; copy and media keep separate gates.'), tone: snapshot.approvalState === 'approved' ? 'positive' as Tone : snapshot.approvalState === 'blocked' ? 'danger' as Tone : 'warning' as Tone },
