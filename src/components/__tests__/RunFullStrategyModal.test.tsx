...
describe('RunFullStrategyModal preflight', () => {
  ...
  it('uses four explicit gates and makes no generation request before final confirmation', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Review cost — 14 credits' }))
  ...
  it('starts exactly one generation request from the final confirmation action', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', { name: 'Review cost — 14 credits' }))

    const finalAction = await screen.findByRole('button', { name: 'Confirm and generate strategy — 14 credits' })
